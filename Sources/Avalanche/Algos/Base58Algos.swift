...
public struct Base58Algos {
    ...
    public func b58(
        data: Data,
        alphabet: [UInt8] = Self.b58Alphabet
    ) -> String {
        var x = BigUInt(data)
    ...
    public func from(cb58: String) -> Data? {
        guard let b58data = from(b58: cb58) else {
            return nil
        }
        print("DATA", b58data)
        ...
        print("CS", checksum)
        let calculated = SHA2.hash(type: .sha256, bytes: Array(prefix))
        print("CALC", calculated.suffix(Self.checksumLength))
    ...
    public func cb58(data: Data) -> String {
